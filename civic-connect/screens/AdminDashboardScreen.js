...
const AdminDashboardScreen = () => {
  ...
  // ✅ Fetch complaints only to show summary counts
  const fetchComplaintsSummary = useCallback(async () => {
    try {
      ...
      Alert.alert("Error", "Failed to load complaint summary");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  return (
    <FlatList
      data={[]} // No complaint list shown here
      keyExtractor={(item) => item._id}
      ListHeaderComponent={
        <>
          {/* 🏠 Dashboard Summary */}
          ...
          {/* 🔄 Refresh Button */}
          <View style={styles.refreshSummaryContainer}>
            <TouchableOpacity
            ...
            >
              {refreshing ? (
                ...
                <>
                  ...
                  <Text style={styles.refreshSummaryText}>Refresh Complaints</Text>
                </>
              )}
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.viewComplaintsContainer}>
            <TouchableOpacity
  ...
  onPress={() =>
    navigation.navigate("AdminComplaintsList", {
      complaints: complaints.filter((c) => {
        let matches = true;
        if (statusFilter !== "All") matches = matches && c.status === statusFilter;
        if (issueTypeFilter !== "All") matches = matches && c.issueType === issueTypeFilter;
        if (wardFilter !== "All") matches = matches && c.ward === wardFilter;
        if (locationFilter !== "All") matches = matches && c.location === locationFilter;
        if (searchQuery.trim())
          matches =
            matches &&
            (c._id.includes(searchQuery) ||
...
>
  <Text style={styles.viewComplaintsText}>📋 View Complaints</Text>
</TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  // 🔄 Refresh summary button styles
  refreshSummaryContainer: {
    alignItems: "center",
    marginBottom: 12,
  },
  ...
  viewComplaintsContainer: { alignItems: "center", marginBottom: 10 },
